...
#define DEVICE_TOKEN_PREF_KEY @"device_token"
...
+ (void)initialize {
    ...
    NSString * deviceToken = [defaults stringForKey:DEVICE_TOKEN_PREF_KEY];
    ...
    if([token length] > 0) {
        ...
        _defaultSession.deviceToken = deviceToken;
    }
}
...
+ (void)setDefaultSession:(IQSession*)defaultSession {
    if(![_defaultSession isEqual:defaultSession]) {
        ...
        if (_defaultSession) {
            ...
            [defaults setValue:_defaultSession.deviceToken forKey:DEVICE_TOKEN_PREF_KEY];
            ...
            [defaults removeObjectForKey:DEVICE_TOKEN_PREF_KEY];
